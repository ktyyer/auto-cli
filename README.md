...
**恢复 /auto 直接执行方式 + 斜杠命令规范化**：
- 删除 CLI fallback 调用（移除 `Bash("auto run ...") `），恢复直接在 Claude Code 中执行工作流
- 斜杠命令仅保留 `/auto` 与 `/auto:*`，移除顶层重复命令路径，并避免本地/全局双份注册
- `create-hook` / `doctor` / `learn` / `status` 统一放入 `commands/auto/`
- 保留所有 PHASE 优化：Skill 注入、消息累积器、验证持久化、知识沉淀增强等
- 测试全绿，lint 0 错误
