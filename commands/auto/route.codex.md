...
如果本次声明使用了 `knowledge-reuse` gate，优先写出至少 1 条 `routeHintsUsed` 或等价知识引用，避免只口头说“复用了 insights / feedback”。
能引用真实目标时，优先写真实存在的 insight 标题、feedback key 或历史 runId，而不是占位或模糊描述。
