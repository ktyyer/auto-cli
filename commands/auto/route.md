...
3. **反馈加权**：读取 `.auto/feedback/agents.json`，对已有记录的 agent 计算 `adjusted_priority = base_priority + (successRate - 0.9) * 20 + (totalCalls > 3 ? 5 : 0)`。样本不足（`totalCalls` < 3）、文件不存在或为空时，该 agent bonus = 0（静默降级为纯静态路由）——单次调用的 `successRate` 不参与加权
