...
| <=3文件 且 无架构变更 | **轻量模式** | PHASE 1 → PHASE 2(简化) → PHASE 4(简化) → PHASE 6 |
...
1. 检测技术栈（读 package.json / pom.xml / go.mod 等）
2. 列出可用能力（commands, agents, skills, hooks 的元数据）
3. 收集源码结构（优先读 REPO_MAP.md，无则扫描 src/）
4. **Router 推荐**：参见 `/auto:route` 命令的 Agent 匹配表，或查看 `src/router/agent-registry.js`
...
如果 `auto` CLI 可用，运行 `auto save insight` 保存踩坑记录和成功模式。
如果 CLI 不可用，在对话总结中标注"CLI 未安装，知识未持久化"。
