...
> **一个命令，自动完成所有事情**
>
> 本文件是 `/auto` 的实际执行协议（用户通过 `/auto` 触发时 Claude Code 加载此文件）。
> 设计文档见 `plugins/builtin/auto-core.md`，两者逻辑一致，修改时需同步更新。
...
│                                                          │
│  【能力清单收集】（健壮化：每个目录单独扫描 + 存在性检查）│
│  Glob("$HOME/.claude/commands/auto/*.md")               │
│    → 如文件数量 > 0：读取每个 frontmatter               │
│    → 如目录不存在或为空：输出 ⚠️ WARNING                │
...
│    → 如含 YOUR_*_HERE 占位符：标记 ⚠️ 需配置 API Key  │
│  Glob("$HOME/.claude/hooks/*.json")                     │
│    → 读取 JSON，统计 hook 类型数量                       │
│                                                          │
│  【用户项目代码扫描】（用于代码风格锚点）                 │
...
│  🔴 红色：目录不存在（核心能力缺失警告）                 │
│                                                          │
│  【能力清单透明输出】（以 Markdown 表格输出给用户）       │
│  输出：Commands · Agents · Plugins · Skills · MCP · Hooks 各列│
│  MCP 含 ⚠️ 标记（如需配置 API Key）                      │
│                                                          │
...
│       4. 遵循五大原则：原子化、黑盒验收、现代工程化、   │
│          代码风格继承、能力编排透明                     │
│       5. 输出顺序：透明度摘要 → 推理日志三段 → Quest Map → 文件校验 │
...
│                                                          │
│  每关执行流程（不论哪种模式）：                           │
│  0. 如 Quest Map > 5 关，激活 Focus Chain：              │
│     Bash("mkdir -p .auto/state") → 创建焦点文件          │
...
🛠️ **选定能力**：本关使用的具体 Agent/Skill/MCP/直接编码。

💡 **选择理由**：（必填）说明为什么选择这个能力组合，而非其他。
...
| 扫描目标 | 扫描路径 | 获取信息 |
|---------|---------|---------|
| **命令** | `$HOME/.claude/commands/auto/*.md` | frontmatter description → 可用命令列表 |
| **Agent** | `$HOME/.claude/agents/*.md` | frontmatter name + description + tools → 可用 Agent |
| **插件** | `$HOME/.claude/plugins/**/*.md` | frontmatter name + description → 插件能力 |
| **Skill** | `$HOME/.claude/skills/**/*.md` | frontmatter name + description → 领域知识库 |
| **MCP** | `$HOME/.claude/mcp-configs/*.json` | 已配置的外部服务 |
| **Hook** | `$HOME/.claude/hooks/*.json` | 已配置的自动化门禁 |
...
### 第2步完成后（quest-designer 输出 — 透明度完整版）

quest-designer 依次输出三段推理日志 + Quest Map：

```markdown
## 📊 能力发现摘要
共扫描能力总数：N 个 | 高匹配：M 个 | 中匹配：K 个
...
⚠️ **如存在代码风格参考文件缺失**，会在 Quest Map 后输出：
```markdown
## ⚠️ 代码风格参考文件校验
⚠️ Quest 2.3: src/components/Foo.tsx — 不存在，请确认路径
```
