...
将 PHASE 1 收集的**完整原始数据**交给 quest-designer v3（不做预筛选）。

quest-designer v3 核心升级：**合约驱动**（跨 Quest 接口协议） + **实现蓝图**（方法签名 + 代码片段） + **风险分层**（🔴🟡🟢） + **反模式警告**（基于代码分析的"不要做"） + **回滚方案**

```
Agent({
  subagent_type: "quest-designer",
  prompt: "你是 quest-designer v3。以下是项目完整上下文和能力清单：
...
严格按你的 7 步工作流执行。v3 关键要求：
- 第2步必须 Read 3-8 个核心文件（分层读取），提取代码片段锚点
- 第3步必须先定义合约(CONTRACT)再拆分 Quest，做全链路依赖分析
- 第4步每个 Quest 必须包含：实现蓝图(🧩) + 反模式警告 + 风险等级 + 回滚方案
- 第5步合约一致性校验（跨 Quest 类型一致性）+ 15项自验证 >= 10/15
- 第6步路径即时校验（设计过程中验证，不事后补）
输出 Quest Map，等待用户确认。"
})
```

> **关键**：quest-designer v3 的合约系统确保跨 Quest 类型一致，实现蓝图消除执行歧义，反模式警告避免风格偏差。
> 产出后等待用户确认，可迭代修改。
