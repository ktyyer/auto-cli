...
检索方式：先读 `.auto/cache/insight-index.json` 反查关键词，缺失时降级 grep。命中条目记入 `selection.routeHintsUsed`。
...
产出标准 LearnCard（≤5 行，原子化可检索，标注 scope），按类别分发到 `.auto/insights/` 对应文件。硬约束：`scope: stack|universal` 额外写入 `skills.json` 的 `portablePatterns`。
