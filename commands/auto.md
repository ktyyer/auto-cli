...
1. **提取检索词**：从用户需求 + 技术栈 + 策略中提取 3-5 个关键词
2. **按优先级扫描 insight 文件**：
   - `traps.md`（最高优先）— 匹配 `**标签**` / `**触发条件**` 字段，注入 `QuestMap.pitfalls`
   - `patterns.md` — 匹配标签，注入 `QuestMap.knowledgeHints`
   - `decisions.md` — 匹配标签，注入 `QuestMap.decisionNotes`
   - `prompts.md` — 仅当策略=探索时按标签匹配
3. **匹配规则**：关键词命中任一 insight 条目标签即纳入，置信度 low 的条目仅参考不注入
4. **反馈叠加**：读取 `.auto/feedback/agents.json` 和 `.auto/feedback/skills.json`，已有负向信号的 agent/skill 降优先
5. **输出**：匹配到的 insight 以 `[insight:<file>#<title>]` 格式记入 `selection.routeHintsUsed`
