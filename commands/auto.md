...
7. **每轮可续接** — 需跨会话时补充 `session-continuity.md`
...
| 重构 | `build` + `test` + `coverage` + `security` + `adversarial` + `self-verification` + `self-critique` + `production-governance` + `skill-activation` + `knowledge-reuse`(full) + `knowledge-distribution` + `clean-state` |
