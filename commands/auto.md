...
输入你的需求，`/auto` 会自动完成：需求分析 → 复杂度评估 → 规划设计 → 编码实现 → 测试验证 → 代码审查 → Git 提交 → 经验沉淀。
...
`/auto` 是**超级命令**，内部自动调用所有能力。精细控制时可使用：
...
```bash
/auto 写代码          # 太模糊
/auto 重构整个系统      # 太复杂，建议用 /auto:deep-plan
```
