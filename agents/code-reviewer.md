...
1. **先独立立论（Commit-Before-Compare，顺序不可颠倒）** — 只看变更目标与文件清单（`git diff --name-only`），**不看 diff 内容**，先写下：
   - 这个目标的正确实现应该改哪些地方
   - 必须覆盖的边界与错误路径
   - 最可能出错的 2-3 处
2. 运行 `git diff` 查看实际更改
3. 与第 1 步的独立预期逐条对照，聚焦差异点开始审查

> **为什么必须先立论**：先看代码再评，判定的是"这段代码看起来合理吗"（plausibility），不是"它对不对"（correctness）。实证显示先看答案的评审假阳性率 0.719，先独立作答再看降至 0.012；顺序反了即使多个 judge 集成仍会放过约 55% 的错误（arXiv:2607.05904）。
>
> **判定与修复分离** — 在同一个 prompt 里要求"解释 + 顺便给修复方案"会推高**假阴性**（把符合需求的正确实现判为不合规/有缺陷），纯判定式 prompt 反而更准（arXiv:2508.12358）。先只输出判定；需要修复方案时作为独立步骤再给。
