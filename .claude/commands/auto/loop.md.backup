...
`/aimax:loop` 用于执行“可恢复的复杂任务编排”。它不是直接写代码，而是先建立状态机执行框架，再推进各步骤。
...
- `/aimax:plan`：先得到高层分解
- `/aimax:loop`：执行状态机编排
- `/aimax:tdd`：在 EXECUTE 阶段保证回归安全
- `/aimax:evolve`：在 VERIFY/SUMMARIZE 阶段评估收敛
