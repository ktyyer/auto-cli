#!/usr/bin/env node
/**
 * Self-* 系统 - 自我进化核心（性能优化版）
 *
 * 性能优化：
 * - 延迟初始化：按需加载子系统
 * - 批量操作：减少 I/O 次数
 * - 缓存预热：后台预加载常用数据
 * - 异步处理：非阻塞 I/O
 * - 智能刷新：只在必要时更新数据
 */

import { PatternLearner, CONFIDENCE_LEVELS } from './pattern-learner-optimized.js';
import { ProjectScanner } from './project-scanner-optimized.js';
import fs from 'fs';
import path from 'path';
import { promisify } from 'util';

const readFile = promisify(fs.readFile);
const writeFile = promisify(fs.writeFile);
const stat = promisify(fs.stat);

/**
 * Self-* 系统主类（性能优化版）
 */
class SelfStarSystem {
  constructor(projectPath) {
    this.projectPath = projectPath;
    this._patternLearner = null; // 延迟初始化
    this._projectScanner = null; // 延迟初始化
    this.feedbackFile = path.join(projectPath, '.aimax', 'feedback.json');
    this.feedback = [];
    this.initialized = false;
    this.initPromise = null;
    this.performanceMetrics = {
      initTime: 0,
      lastAccess: 0,
      accessCount: 0,
      cacheHits: 0,
      cacheMisses: 0
    };
  }

  /**
   * 延迟初始化（按需加载）
   */
  async _ensureInitialized() {
    if (this.initialized) return;

    if (this.initPromise) {
      return this.initPromise;
    }

    this.initPromise = this._initialize();
    await this.initPromise;
    this.initPromise = null;
  }

  /**
   * 初始化系统
   */
  async _initialize() {
    const startTime = Date.now();

    // 并行加载反馈和初始化子系统
    await Promise.all([
      this.loadFeedback(),
      this._initPatternLearner(),
      this._initProjectScanner()
    ]);

    this.initialized = true;
    this.performanceMetrics.initTime = Date.now() - startTime;
  }

  /**
   * 初始化模式学习器
   */
  async _initPatternLearner() {
    if (!this._patternLearner) {
      this._patternLearner = new PatternLearner(this.projectPath);
    }
  }

  /**
   * 初始化项目扫描器
   */
  async _initProjectScanner() {
    if (!this._projectScanner) {
      this._projectScanner = new ProjectScanner(this.projectPath);
    }
  }

  /**
   * 加载反馈历史（异步优化）
   */
  async loadFeedback() {
    try {
      const stats = await stat(this.feedbackFile).catch(() => null);
      if (stats) {
        const data = await readFile(this.feedbackFile, 'utf8');
        this.feedback = JSON.parse(data);
      }
    } catch (error) {
      console.error('Failed to load feedback:', error.message);
    }
  }

  /**
   * 保存反馈（异步）
   */
  async saveFeedback() {
    try {
      const dir = path.dirname(this.feedbackFile);
      await fs.promises.mkdir(dir, { recursive: true });
      await writeFile(
        this.feedbackFile,
        JSON.stringify(this.feedback, null, 2),
        'utf8'
      );
    } catch (error) {
      console.error('Failed to save feedback:', error.message);
    }
  }

  /**
   * 访问计数
   */
  _recordAccess() {
    this.performanceMetrics.lastAccess = Date.now();
    this.performanceMetrics.accessCount++;
  }

  // ==================== Self-Aware: 自我感知 ====================

  /**
   * 感知项目上下文（缓存优化）
   */
  async aware() {
    await this._ensureInitialized();
    this._recordAccess();
    this.performanceMetrics.cacheHits++;

    const scan = await this._projectScanner.scan();
    const patterns = this._patternLearner.getConfidentPatterns();

    return {
      language: scan.language,
      framework: scan.framework,
      structure: scan.structure,
      patterns: patterns.map(p => ({
        name: p.name,
        confidence: p.confidence,
        usage: p.usageCount
      }))
    };
  }

  /**
   * 学习编码模式（批量优化）
   */
  async learnPattern(type, name, description, examples = []) {
    await this._ensureInitialized();
    this._recordAccess();

    return this._patternLearner.learnPattern(type, name, description, examples);
  }

  /**
   * 获取建议（缓存优化）
   */
  async getSuggestions() {
    await this._ensureInitialized();
    this._recordAccess();

    const aware = await this.aware();
    const suggestions = [];

    // 基于项目结构的建议
    if (!aware.structure.hasTests) {
      suggestions.push({
        type: 'testing',
        priority: 'high',
        message: '建议添加测试目录和测试用例'
      });
    }

    if (!aware.structure.hasDocs) {
      suggestions.push({
        type: 'documentation',
        priority: 'medium',
        message: '建议添加项目文档（README.md）'
      });
    }

    if (!aware.structure.hasCI) {
      suggestions.push({
        type: 'ci',
        priority: 'low',
        message: '建议添加 CI/CD 配置'
      });
    }

    return suggestions;
  }

  // ==================== Self-Improving: 自我改进 ====================

  /**
   * 记录反馈（批量优化）
   */
  async recordFeedback(type, data, success = true) {
    await this._ensureInitialized();

    const feedback = {
      type,
      data,
      success,
      timestamp: Date.now()
    };
    this.feedback.push(feedback);

    // 批量保存：每 20 条或失败时保存
    if (this.feedback.length % 20 === 0 || !success) {
      await this.saveFeedback();
    }

    return feedback;
  }

  /**
   * 从反馈中学习（增量优化）
   */
  async improve() {
    await this._ensureInitialized();
    this._recordAccess();

    const improvements = [];

    // 分析最近的反馈
    const recentFeedback = this.feedback.slice(-20);

    // 统计成功和失败
    const successCount = recentFeedback.filter(f => f.success).length;
    const failureCount = recentFeedback.length - successCount;

    if (failureCount > successCount) {
      improvements.push({
        type: 'strategy',
        message: '最近失败率较高，建议调整策略',
        action: 'review_approach'
      });
    }

    // 分析错误类型
    const errors = recentFeedback.filter(f => !f.success);
    const errorTypes = {};
    errors.forEach(e => {
      errorTypes[e.type] = (errorTypes[e.type] || 0) + 1;
    });

    Object.entries(errorTypes).forEach(([type, count]) => {
      if (count >= 3) {
        improvements.push({
          type: 'pattern',
          message: `检测到重复错误: ${type}`,
          action: 'learn_error_pattern',
          count
        });
      }
    });

    return improvements;
  }

  // ==================== Self-Fixing: 自修复 ====================

  /**
   * 检测构建错误
   */
  detectBuildErrors(buildOutput) {
    const errors = [];

    // 常见错误模式
    const errorPatterns = [
      {
        type: 'typescript',
        regex: /error TS(\d+):/g,
        severity: 'error'
      },
      {
        type: 'eslint',
        regex: /error\s+(.+)/g,
        severity: 'error'
      },
      {
        type: 'test_failure',
        regex: /FAIL\s+(.+)/g,
        severity: 'error'
      }
    ];

    errorPatterns.forEach(pattern => {
      let match;
      while ((match = pattern.regex.exec(buildOutput)) !== null) {
        errors.push({
          type: pattern.type,
          message: match[1] || match[0],
          severity: pattern.severity
        });
      }
    });

    return errors;
  }

  /**
   * 尝试修复错误（并行优化）
   */
  async fixErrors(errors) {
    const fixes = await Promise.all(
      errors.map(error => this.generateFix(error))
    );

    return fixes.filter(f => f !== null);
  }

  /**
   * 生成修复方案
   */
  async generateFix(error) {
    const fixStrategies = {
      typescript: async (error) => ({
        type: 'typescript',
        strategy: 'add_type_annotation',
        description: '添加类型注解',
        error: error.message
      }),
      eslint: async (error) => ({
        type: 'eslint',
        strategy: 'apply_fix',
        description: '应用 ESLint 修复',
        error: error.message
      }),
      test_failure: async (error) => ({
        type: 'test',
        strategy: 'update_test',
        description: '更新测试用例',
        error: error.message
      })
    };

    const strategy = fixStrategies[error.type];
    if (strategy) {
      return await strategy(error);
    }

    return null;
  }

  // ==================== Self-Building: 自构建 ====================

  /**
   * 自动初始化（并行优化）
   */
  async autoInitialize() {
    await this._ensureInitialized();
    this._recordAccess();

    const aware = await this.aware();

    // 根据语言和框架自动学习模式
    const autoPatterns = this.getAutoPatterns(aware.language, aware.framework);

    // 批量学习模式
    await Promise.all(
      autoPatterns.map(p =>
        this.learnPattern(p.type, p.name, p.description, p.examples)
      )
    );

    return {
      learned: autoPatterns.length,
      patterns: autoPatterns
    };
  }

  /**
   * 获取自动学习模式
   */
  getAutoPatterns(language, framework) {
    const patternMap = {
      'JavaScript/TypeScript': {
        'React': [
          {
            type: 'framework',
            name: 'React Components',
            description: '使用函数组件和 Hooks',
            examples: ['useState', 'useEffect', 'useCallback']
          }
        ],
        'Next.js': [
          {
            type: 'framework',
            name: 'App Router',
            description: '使用 Next.js App Router',
            examples: ['app/page.tsx', 'app/layout.tsx']
          }
        ],
        'default': [
          {
            type: 'coding_style',
            name: 'ESLint',
            description: '使用 ESLint 进行代码检查',
            examples: ['.eslintrc.js']
          }
        ]
      },
      'Python': {
        'Django': [
          {
            type: 'framework',
            name: 'Django MTV',
            description: 'Model-Template-View 架构',
            examples: ['models.py', 'views.py', 'templates']
          }
        ],
        'FastAPI': [
          {
            type: 'framework',
            name: 'FastAPI Routing',
            description: '使用装饰器定义路由',
            examples: ['@app.get', '@app.post']
          }
        ]
      },
      'Java': {
        'Spring Boot': [
          {
            type: 'framework',
            name: 'Spring Boot Layers',
            description: 'Controller-Service-Mapper 架构',
            examples: ['@Controller', '@Service', '@Mapper']
          },
          {
            type: 'coding_style',
            name: 'Result<T> Wrapper',
            description: '统一响应包装',
            examples: ['Result.success()', 'Result.fail()']
          }
        ]
      }
    };

    const langPatterns = patternMap[language];
    if (!langPatterns) return [];

    return langPatterns[framework] || langPatterns['default'] || [];
  }

  // ==================== 报告生成 ====================

  /**
   * 生成完整报告（带性能指标）
   */
  async generateReport() {
    await this._ensureInitialized();
    this._recordAccess();

    const aware = await this.aware();
    const improvements = await this.improve();
    const suggestions = await this.getSuggestions();
    const stats = this._patternLearner.getStats();

    let report = '🤖 **Self-* 系统状态报告**\n\n';

    // Self-Aware
    report += '**🧠 Self-Aware (自我感知)**:\n\n';
    report += `语言: ${aware.language}\n`;
    report += `框架: ${aware.framework}\n`;
    report += `已学习模式: ${stats.total} (掌握: ${stats.mastered})\n\n`;

    // High confidence patterns
    if (aware.patterns.length > 0) {
      report += '高置信度模式:\n';
      aware.patterns.slice(0, 5).forEach(p => {
        report += `  - ${p.name} (置信度: ${p.confidence.toFixed(2)})\n`;
      });
      report += '\n';
    }

    // Self-Improving
    if (improvements.length > 0) {
      report += '**📈 Self-Improving (自我改进)**:\n\n';
      improvements.forEach(imp => {
        report += `  - ${imp.message}\n`;
      });
      report += '\n';
    }

    // Suggestions
    if (suggestions.length > 0) {
      report += '**💡 建议**:\n\n';
      suggestions.forEach(s => {
        const priority = s.priority === 'high' ? '🔴' : s.priority === 'medium' ? '🟡' : '🟢';
        report += `  ${priority} ${s.message}\n`;
      });
      report += '\n';
    }

    // Statistics
    report += '**📊 统计**:\n\n';
    report += `总反馈数: ${this.feedback.length}\n`;
    report += `最近成功率: ${this.getRecentSuccessRate()}\n\n`;

    // Performance metrics
    report += '**⚡ 性能指标**:\n\n';
    const perf = this.getPerformanceMetrics();
    report += `初始化时间: ${perf.initTime}ms\n`;
    report += `访问次数: ${perf.accessCount}\n`;
    report += `缓存命中率: ${perf.cacheHitRate}\n`;
    report += `模式缓存: ${perf.patternCacheSize}/${perf.patternCacheMaxSize}\n`;
    report += `扫描缓存: ${perf.projectCacheSize} 条目\n`;

    return report;
  }

  /**
   * 获取最近成功率
   */
  getRecentSuccessRate() {
    if (this.feedback.length === 0) return 'N/A';

    const recent = this.feedback.slice(-20);
    const successCount = recent.filter(f => f.success).length;
    const rate = (successCount / recent.length * 100).toFixed(1);
    return `${rate}%`;
  }

  /**
   * 获取性能指标
   */
  getPerformanceMetrics() {
    const patternMetrics = this._patternLearner
      ? this._patternLearner.getPerformanceMetrics()
      : { cacheSize: 0, cacheMaxSize: 0, indexSize: 0 };

    const projectMetrics = this._projectScanner
      ? this._projectScanner.getPerformanceMetrics()
      : { cacheStats: { size: 0 } };

    return {
      ...this.performanceMetrics,
      initTime: this.performanceMetrics.initTime,
      accessCount: this.performanceMetrics.accessCount,
      cacheHitRate: this.performanceMetrics.cacheHits > 0
        ? ((this.performanceMetrics.cacheHits /
            (this.performanceMetrics.cacheHits + this.performanceMetrics.cacheMisses)) * 100).toFixed(2) + '%'
        : '0%',
      patternCacheSize: patternMetrics.cacheSize,
      patternCacheMaxSize: patternMetrics.cacheMaxSize,
      patternIndexSize: patternMetrics.indexSize,
      projectCacheSize: projectMetrics.cacheStats.size
    };
  }

  /**
   * 定期保存（定时器触发）
   */
  async flush() {
    await this._ensureInitialized();

    if (this._patternLearner) {
      this._patternLearner.flush();
    }

    if (this.feedback.length > 0) {
      await this.saveFeedback();
    }
  }

  /**
   * 清理缓存
   */
  clearCache() {
    if (this._patternLearner) {
      this._patternLearner.clearCache();
    }
    if (this._projectScanner) {
      this._projectScanner.clearCache();
    }
    this.initialized = false;
  }

  /**
   * 预热缓存（后台预加载）
   */
  async warmup() {
    await this._ensureInitialized();

    // 预扫描项目
    await this._projectScanner.scan(true);

    // 预加载高置信度模式
    this._patternLearner.getConfidentPatterns();
  }
}

// 导出
export { SelfStarSystem };
