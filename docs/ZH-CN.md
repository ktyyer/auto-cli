...
### 技巧 6：自动上下文注入（v0.10.0 新增）

基于 linux.do 社区最佳实践，Auto CLI 现在支持自动上下文注入：

```bash
# AI 会根据任务自动选择合适的上下文预设
/auto 实现用户注册功能    # -> 自动选择"实现模式"
/auto 修复登录页面报错     # -> 自动选择"修复模式"
/auto 审查 PR 代码质量     # -> 自动选择"审查模式"
```
...
### 技巧 7：探索-规划-编码工作流（v0.10.0 新增）

遵循 linux.do 社区推荐的 "Explore first, then plan, then code" 工作流：

```
阶段 1：探索 → 全面了解项目结构和依赖
阶段 2：规划 → 基于探索结果制定详细计划
阶段 3：编码 → 按计划逐步实现
```
