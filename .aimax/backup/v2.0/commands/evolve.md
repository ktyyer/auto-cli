...
`/aimax:evolve` 用于把“改进”变成可验证的工程闭环：
...
```bash
/aimax:evolve 为这个仓库建立可复用的回归门禁，覆盖 Node + Python 子项目
```
...
- 先用 `/aimax:plan` 定义大任务分解
- 用 `/aimax:tdd` 产出稳定测试护栏
- 用 `/aimax:code-review` 做多维审查
- 用 `/aimax:evolve` 驱动持续迭代与门禁收敛
